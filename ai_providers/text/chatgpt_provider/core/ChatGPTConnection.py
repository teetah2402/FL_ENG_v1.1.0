...
class ChatGPTConnection:
    ...
    def configure(self):
        # 1. Jalur Saraf Standar (Proteksi MockKernel)
        try:
        ...
        # 2. Jalur Saraf Lingkungan (OS Environ)
        if not self.api_key:
            self.api_key = os.environ.get("OPENAI_API_KEY")

        # 3. Jalur Saraf Bypass (Direct DB Access)
        if not self.api_key:
            self.api_key = self._fetch_from_db("OPENAI_API_KEY")

        if not self.api_key:
            # Proteksi pemanggilan log pada MockKernel
