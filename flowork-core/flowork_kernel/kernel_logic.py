...
# File NAME : C:\flowork-nano\flowork-core\flowork_kernel\kernel_logic.py total lines 655 
########################################################################
...
class Kernel:
    ...
    license_tier: str = "OpenCore" # Forced for OpenCore functionality
    is_premium: bool = True # Forced for OpenCore functionality
...
-----END PUBLIC KEY-----"""
    ...
    def __init__(self, project_root_path: str):
        ...
        if external_data_path and os.path.isdir(external_data_path):
            ...
            self.apps_path = os.path.join(self.data_path, "apps")
            ...
            self.apps_path = os.path.join(self.project_root_path, "apps")
            self.widgets_path = self.apps_path
    ...
    def _load_services_from_manifest(self):
        ...
        try:
            ...
            for service_id in essential_order:
                if service_id in ZOMBIE_SERVICES:
                     continue
            ...
            for service_config in all_service_configs:
                if service_config["id"] in ZOMBIE_SERVICES:
                     continue
    ...
    def _load_service(self, service_config: Dict[str, str]):
        ...
        try:
            ...
            critical_services = [
                ...
                "startup_service",
                "gateway_connector_service", "api_server_service",
            ]
    ...
    def hot_reload_components(self):
        ...
        am = self.get_service("app_manager_service", is_system_call=True)
        if am:
            am.discover_and_load_apps()
    ...
    def activate_license_online(self, full_license_content: dict):
        return False, "Online activation disabled for OpenCore."

    def deactivate_license_on_server(self):
        return False, "Online deactivation disabled for OpenCore."
