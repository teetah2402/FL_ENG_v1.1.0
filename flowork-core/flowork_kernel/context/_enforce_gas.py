...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\context\_enforce_gas.py total lines 63 
...
def run(hub, cost: int, operation_name: str):
    if hub.kill_flag:
        raise Exception(f"Agent (ID: {hub.agent_id}) is terminated. Cannot perform '{operation_name}'.")
    if not hub.fac_runtime.consume_gas(cost):
        hub.kill_flag = True
        hub.timeline.log(event_type='agent_killed', data={'reason': 'OUT_OF_GAS', 'failed_operation': operation_name, 'gas_spent': hub.fac_runtime.get_gas_spent()})
        raise Exception(f"Out of Gas! Agent (ID: {hub.agent_id}) terminated. Failed on '{operation_name}'.")
    hub.timeline.log(event_type='gas_spent', data={'operation': operation_name, 'cost': cost, 'total_spent': hub.fac_runtime.get_gas_spent()})
