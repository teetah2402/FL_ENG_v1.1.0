...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\context\episodic_read.py total lines 64 
...
def run(hub, key: str) -> dict | None:
    hub.execute_sync('_enforce_gas', GAS_COSTS['EPISODIC_READ'], 'episodic_read')
    try:
        data = hub.episodic.read(key)
        hub.timeline.log('episodic_read', {'key': key, 'found': data is not None})
        return data
    except Exception as e:
        hub.timeline.log('episodic_read', {'key': key, 'success': False, 'error': str(e)})
        raise
