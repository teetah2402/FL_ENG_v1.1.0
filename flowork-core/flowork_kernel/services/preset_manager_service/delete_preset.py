...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\preset_manager_service\delete_preset.py total lines 25 
...
def run(hub, name: str, user_id: str) -> bool:
    wf_path = hub.execute_sync('_get_preset_workflow_path', user_id, name)
    if os.path.exists(wf_path):
        shutil.rmtree(wf_path)
        hub.execute_sync('_sync_trigger_rules_for_preset', name, None, user_id, is_delete=True)
        return True
    return False
