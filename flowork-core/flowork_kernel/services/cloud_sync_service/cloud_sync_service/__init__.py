...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\cloud_sync_service\cloud_sync_service\__init__.py total lines 157 
...
class CloudSyncService(BaseService):
    ...
    def _create_zip_from_user_data(self, user_id: str) -> str:
        ...
        self.logger(f'Successfully created zip archive at: {output_zip_path}.zip', 'SUCCESS')
    ...
    def encrypt_and_upload_backup(self, user_id: str, password: str):
        ...
        try:
            ...
            self.logger(f'Upload successful! Gateway responded: {response.json()}', 'SUCCESS')
    ...
    def download_and_decrypt_backup(self, user_id: str, password: str):
        ...
        try:
            ...
            self.logger(f"Restore process for user '{user_id}' completed successfully!", 'SUCCESS')
            self.kernel.hot_reload_components()
            return (True, 'Restore completed successfully.')
        except requests.exceptions.HTTPError as e:
            if e.response.status_code == 404:
                self.logger(f"No backup found on the server for user '{user_id}'.", 'WARN')
                return (False, 'No backup found on the server.')
            else:
                self.logger(f'An HTTP error occurred during the restore process: {e.response.text}', 'CRITICAL')
                return (False, f'Server error: {e.response.text}')
        except Exception as e:
            self.logger(f'An error occurred during the restore process: {e}', 'CRITICAL')
            import traceback
            self.logger(traceback.format_exc(), 'DEBUG')
            return (False, str(e))
