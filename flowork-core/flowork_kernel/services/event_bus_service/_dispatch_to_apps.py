...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\event_bus_service\_dispatch_to_apps.py total lines 62
##1. Dynamic Component Discovery (DCD): Hub wajib melakukan scanning file secara otomatis.
##2. Lazy Loading: Modul hanya di-import ke RAM saat dipanggil (On-Demand).
##3. Atomic Isolation: 1 File = 1 Fungsi dengan nama file yang identik with nama fungsi aslinya.
...
def run(hub, event_name, payload):
    ...
    """
    # Mengambil daftar (app_id, action_name) yang berlangganan event ini
    app_targets = hub.app_subscriptions.get(event_name, [])
    if not app_targets:
        return

    # [LazyHub] Resolve AppRuntime cache jika belum tersedia
    ...
    if hub._app_runtime_cache:
        for (app_id, action_name) in app_targets:
            try:
                # [SECURITY] Cegah Loop Saraf: Jangan kirim balik ke pengirim asli
                ...
                # [PHASE 3 Integration] Gunakan trigger_event_handler (Lazarus Aware)
                # Memastikan App dibangunkan dulu jika sedang Ghost mode
                if loop and loop.is_running():
                    # Menjalankan secara asinkron di dalam event loop utama
                    loop.create_task(hub._app_runtime_cache.trigger_event_handler(app_id, action_name, payload))
                else:
                    # Jalur darurat via Thread jika loop tidak aktif
