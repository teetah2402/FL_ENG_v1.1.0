...
def run(hub, query: str, user_id: str, limit: int = 3, threshold: float = 0.7):
    ...
    # 1. Dapatkan vektor dari query user
    query_vector = hub.ai_manager.execute_sync("get_embedding", query)
    if not query_vector:
        return []

    # 2. Ambil semua data vektor milik user dari database
    # Sesuai arsitektur, kita gunakan database_service
    sql = """
    ...
    # 3. Hitung similarity di memori (Python logic)
    for row in rows:
        content = row[0]
        source = row[1]
