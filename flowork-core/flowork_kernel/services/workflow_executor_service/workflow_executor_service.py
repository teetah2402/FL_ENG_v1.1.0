...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\workflow_executor_service\workflow_executor_service.py total lines 744 
########################################################################
...
class WorkflowExecutorService(BaseService):
    ...
    def _watchdog_loop(self):
        ...
        while True:
            try:
                time.sleep(3)
                if not self.db_service: continue
        ...
        try:
            ...
            for (target_node_id, source_handle) in targets:
                ...
                for (dep_source_id,) in dependencies:
                    cursor.execute(
                        "SELECT output_data, status FROM Jobs WHERE execution_id = ? AND node_id = ? AND status IN ('DONE', 'FAILED') ORDER BY finished_at DESC LIMIT 1",
                        (execution_id, dep_source_id)
                    )
                    row = cursor.fetchone()
                ...
                if all_met:
                    ...
                    cursor.execute(
                        "INSERT INTO Jobs (job_id, execution_id, node_id, status, input_data, workflow_id, user_id) VALUES (?, ?, ?, 'PENDING', ?, ?, ?)",
                        (new_job_id, execution_id, target_node_id, safe_input, workflow_id, user_id)
                    )
                    conn.commit()
    ...
    async def execute_standalone_node(self, payload: dict):
        ...
        if self.app_manager and raw_node_id in self.app_manager.registry.get("apps", {}).get("data", {}):
            ...
            ctx = {
                ...
                "job_id": forced_job_id or str(uuid.uuid4()),
                "workflow_id": None
            }
        ...
        try:
            ...
            try:
                ...
                cursor.execute(
                    "SELECT node_id, workflow_id FROM Nodes WHERE node_type = ? ORDER BY rowid DESC LIMIT 1",
                    (raw_node_id,)
                )
                row = cursor.fetchone()
                if row:
            ...
            if not workflow_id:
                workflow_id = "00000000-0000-0000-0000-000000000000" # Dummy Workflow ID
