...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\workflow_executor_service\_publish_log.py total lines 37 
...
def run(hub, execution_id: str, node_id: str, message: str, level: str='INFO'):
    ...
    if hub.event_bus:
        try:
            ...
            hub.event_bus.publish('WORKFLOW_LOG_ENTRY', payload, publisher_id='SYSTEM')
            ...
            if hub.gateway_connector:
                hub.gateway_connector.forward_event_to_gateway('WORKFLOW_LOG_ENTRY', 'vip_express', payload)
        except Exception as e:
            hub.logger.error(f'Failed to publish log: {e}')
