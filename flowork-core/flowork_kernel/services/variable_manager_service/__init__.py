########################################################################
# WEBSITE https://flowork.cloud
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\variable_manager_service\__init__.py total lines 100 
##1. Dynamic Component Discovery (DCD): Hub wajib melakukan scanning file secara otomatis.
##2. Lazy Loading: Modul hanya di-import ke RAM saat dipanggil (On-Demand).
##3. Atomic Isolation: 1 File = 1 Fungsi dengan nama file yang identik dengan nama fungsi aslinya.
##4. Zero Logic Mutation: Dilarang merubah alur logika, nama variabel, atau struktur if/try/loop.
########################################################################

from flowork_kernel.services.base_service import BaseService

import importlib
import os
import logging
import asyncio
import threading
from collections import OrderedDict

class VariableManagerService(BaseService):
    MAX_USER_CACHE_SIZE = 100
    SYSTEM_GLOBAL_KEYS = [
        'OPENAI_API_KEY', 'GEMINI_API_KEY', 'ANTHROPIC_API_KEY',
        'FLOWORK_API_KEY', 'STABILITY_API_KEY', 'GROQ_API_KEY',
        'DEEPSEEK_API_KEY'
    ]

    def __init__(self, kernel, service_id: str):
        self.cells = {}
        if not hasattr(self, 'logger'):
            self.logger = logging.getLogger('LazyHub')

        super().__init__(kernel, service_id)

        self.users_data_path = os.path.normpath(os.path.join(self.kernel.data_path, 'users'))
        if not os.path.exists(self.users_data_path):
            try:
                os.makedirs(self.users_data_path, exist_ok=True)
            except Exception:
                pass

        self._variables_data_cache = OrderedDict()
        self._sequential_counters = {}
        self._lock = threading.Lock()

        self.autodiscover_and_sync_variables()

        self.kernel.write_to_log(
            f"Service 'VariableManager' ready. Storage Engine: SQLite (Flowork Core DB)", 'DEBUG'
        )

    def execute_sync(self, cell_name, *args, **kwargs):
        module = self._load_cell(cell_name)
        return module.run(self, *args, **kwargs)

    async def execute_async(self, cell_name, *args, **kwargs):
        module = self._load_cell(cell_name)
        return await module.run(self, *args, **kwargs)

    def _load_cell(self, cell_name):
        if cell_name not in self.cells:
            try:
                module = importlib.import_module(f".{cell_name}", package=__package__)
                self.cells[cell_name] = module
                if hasattr(self, 'logger'):
                    self.logger.info(f"[LazyHub] ✅ Loaded: {cell_name}.py")
            except Exception as e:
                if hasattr(self, 'logger'):
                    self.logger.error(f"[LazyHub] ❌ Failed to load '{cell_name}': {e}")
                raise e
        return self.cells[cell_name]

    def _get_db_connection(self, *args, **kwargs):
        return self.execute_sync('_get_db_connection', *args, **kwargs)

    def autodiscover_and_sync_variables(self, *args, **kwargs):
        return self.execute_sync('autodiscover_and_sync_variables', *args, **kwargs)

    def _get_user_variables_path(self, *args, **kwargs):
        return self.execute_sync('_get_user_variables_path', *args, **kwargs)

    def _load_variables_from_file(self, *args, **kwargs):
        return self.execute_sync('_load_variables_from_file', *args, **kwargs)

    def _save_variables_to_file(self, *args, **kwargs):
        return self.execute_sync('_save_variables_to_file', *args, **kwargs)

    def get_all_variables_for_api(self, *args, **kwargs):
        return self.execute_sync('get_all_variables_for_api', *args, **kwargs)

    def get_variable_metadata(self, *args, **kwargs):
        return self.execute_sync('get_variable_metadata', *args, **kwargs)

    def get_variable(self, *args, **kwargs):
        return self.execute_sync('get_variable', *args, **kwargs)

    def set_variable(self, *args, **kwargs):
        return self.execute_sync('set_variable', *args, **kwargs)

    def delete_variable(self, *args, **kwargs):
        return self.execute_sync('delete_variable', *args, **kwargs)

    def set_variable_enabled_state(self, *args, **kwargs):
        return self.execute_sync('set_variable_enabled_state', *args, **kwargs)
