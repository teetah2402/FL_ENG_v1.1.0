...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\gateway_connector_service\_resolve_home_gateway.py total lines 46 
...
def run(hub) -> str:
    try:
        base_url = hub.gateway_url.replace('/api/v1', '').rstrip('/')
        res = requests.get(f'{base_url}/api/v1/cluster/resolve-home?key={hub.engine_id}', timeout=3.0)
        if res.status_code == 200:
            home_url = res.json().get('url')
            if home_url:
                return home_url
        return hub.gateway_url
    except:
        return hub.gateway_url
