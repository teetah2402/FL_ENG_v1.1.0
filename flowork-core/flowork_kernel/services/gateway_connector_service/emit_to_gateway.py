...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\gateway_connector_service\emit_to_gateway.py total lines 42 
...
async def run(hub, event_name: str, payload: dict):
    try:
        if not hub.sio.connected or '/engine-socket' not in hub.sio.namespaces:
            return
        await hub.sio.emit(event_name, payload, namespace='/engine-socket')
    except Exception as e:
        hub.logger.error(f'Emit error: {e}')
