...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\api_server_service\routes\system_routes.py total lines 233 
########################################################################
...
class SystemRoutes(BaseApiRoute):
        ...
        try:
            ...
            if not diag_app:
                 return self._json_response(
                     {"error": "Diagnostics App (system_doctor) is not installed or available."},
                     status=503
                 )

            self.logger(f"Diagnostics requested via App. ID: {scan_id}, Target: {target_scanner_id}", "INFO")
    ...
    async def handle_addon_upload(self, request):
        ...
        if not app_service:
            return self._json_response(
                {"error": "App/Addon management service is not available."}, status=503
            )
        ...
        if body.get("bundle_data"):
             try:
                 ...
                 return self._json_response({"error": str(inst_err)}, status=500)


        return self._json_response({"error": "Direct component upload deprecated. Use bundle installer."}, status=410)
