...
# File NAME : C:\flowork-nano\flowork-core\flowork_kernel\api_contract.py total lines 309 
########################################################################
...
class BaseModule:
    ...
    def publish_event(self, event_name: str, event_data: Dict[str, Any]):
        if not isinstance(event_data, dict):
            self.logger(f"Wrapping non-dict event data for '{event_name}'", "WARN")
        ...
        if event_bus:
            ...
            try:
                ...
                self.logger(f"EventBus publish failed (safe ignore in worker context): {e}", "WARN")
        else:
            self.logger(f"Event '{event_name}' buffered for bridge transport.", "INFO")


class BaseHybridApp(ABC):
    """
    Standard interface for all Flowork Hybrid Apps.
    Replaces the legacy BaseDashboardWidget.
    """
    def __init__(self, kernel, app_id: str):
        self.kernel = kernel
        self.loc = self.kernel.get_service("localization_manager")
        self.app_id = app_id
    ...
    def on_app_load(self):
        """Called when the AppManager initializes this app instance."""
        pass

    @abstractmethod
    def on_app_destroy(self):
        """Called when the app is being unloaded or deleted."""
        pass

    @abstractmethod
    def refresh_content(self):
        """Request the app to refresh its internal logic/data."""
        pass

    @abstractmethod
    def get_app_state(self) -> dict:
        """Fetch current persistent data for this app."""
        return {}

    @abstractmethod
    def load_app_state(self, state: dict):
        """Restore previous persistent data."""
        pass


class BaseDashboardWidget(BaseHybridApp):
    """
    Zombie compatibility layer.
    Redirects AppManager calls to legacy Widget methods.
    """
    def __init__(self, kernel, widget_id: str):
        super().__init__(kernel, app_id=widget_id)
        self.widget_id = widget_id # Legacy reference

    def on_app_load(self):
        return self.on_widget_load()

    def on_app_destroy(self):
        return self.on_widget_destroy()

    def get_app_state(self):
        return self.get_widget_state()

    def load_app_state(self, state):
        return self.load_widget_state(state)

    def on_widget_load(self): pass
    def on_widget_destroy(self): pass
    def get_widget_state(self): return {}
    def load_widget_state(self, state): pass
