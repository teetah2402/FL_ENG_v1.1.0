...
# File NAME : C:\FLOWORK\flowork-core\app\executor\runner.py total lines 295
...
async def main():
    ...
    # [Fase 1] Path Injection: Membisikkan alamat library dari Warehouse
    try:
        ...
        if lib_paths:
            ...
            for p in lib_paths:
                if p not in sys.path:
                    # Dahulukan library Warehouse agar tidak bentrok dengan lib sistem Docker
                    sys.path.insert(0, p)
    ...
    # [Fase 3] Start Socket Server untuk Immortal Bridge
...
async def handle_socket_client(reader, writer):
    ...
    try:
        ...
        # Support payload lama (data) dan baru (params)
...
def load_app_modules(app_path, app_id):
    ...
    try:
        ...
        # [Fase 2] Namespacing: Interceptor untuk mencegah App mematikan Core
        ...
        # [Fase 4] The Iron Bank Proxy (Isolated Kernel)
        class IsolatedKernel:
            ...
            class DatabaseStub:
                # [Fase 4] Database Proxy: Tidak boleh ada direct SQL
...
async def process_action(action, payload):
    ...
    try:
        # [NEW] Smart Argument Injection: Cek apakah handler butuh KERNEL_STUB (hub)
        sig = inspect.signature(handler)
        params_count = len(sig.parameters)

        # Eksekusi dengan dukungan Hub Injection (seperti di Kernel)
        if params_count >= 2:
