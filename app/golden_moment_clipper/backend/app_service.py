...
# File NAME : C:\FLOWORK\app\golden_moment_clipper\backend\app_service.py total lines 472 
########################################################################
            ...
            if data_root.exists():
                for entry in os.scandir(data_root):
                    if entry.is_dir() and entry.name != "system":
                        return entry.name # Nemu user asli!
        except: pass
        return "system"
        ...
        if self.muscle:
            try:
                ...
                if res.get("status") == "success":
                    ...
                    self.active_jobs[job_id]["status"] = "completed" # Muscle manages its own completion logs
                    return
                else:
        ...
        for idx, vid_path in enumerate(videos, 1):
            ...
            try:
                ...
                for i, (start, end) in enumerate(segments):
                    ...
                    self._ffmpeg_render(work, final, config.get("resize_mode", "podcast_split"), config.get("watermark_text", ""), ass, None)
