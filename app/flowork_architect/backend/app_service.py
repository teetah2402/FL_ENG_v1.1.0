...
# File NAME : C:\FLOWORK\app\flowork_architect\backend\app_service.py total lines 514 
########################################################################
...
class ArchitectService(BaseAppService):
    """
    THE CORTEX MANAGER (IMMORTAL EDITION)
    ...
    def __init__(self, kernel, service_id):
        ...
        self.config_root_path = os.environ.get("SCAN_ROOT_PATH", "/master_workspace")
        self.auto_delete_zombies = os.environ.get("AUTO_DELETE_ZOMBIES", "false").lower() == "true"
        self.analysis_depth = os.environ.get("ANALYSIS_DEPTH", "deep")

        if os.path.exists(self.config_root_path):
            self.root_dir = self.config_root_path
        elif os.path.exists("/master_workspace"):
    ...
    def zombie_hunter_job(self, payload):
        """Dipanggil oleh Core Scheduler setiap Minggu jam 3 pagi."""
        self.logger.info("⏰ [Scheduler] Running Scheduled Zombie Hunter...")
        self.force_sync()

        if self.auto_delete_zombies:
             self.logger.info("💀 [Scheduler] Auto-Delete Zombies is ENABLED. Cleaning up...")
             zombie_count = ArchitectService._SHARED_DATA.get("graph", {}).get("zombie_count", 0)
             self.logger.info(f"💀 [Scheduler] Found {zombie_count} zombies (Auto-delete pending implementation for safety).")

        return {"status": "success", "message": "Scheduled scan completed."}
    ...
    def _run_scan_worker(self):
        ...
        try:
            self.muscle.execute_app(
                ...
                action="scan",
                params={"root_dir": self.root_dir, "depth": self.analysis_depth},
                user_id="system"
            )
        except Exception as e:
