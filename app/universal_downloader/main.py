...
# File NAME : C:\FLOWORK\app\universal_downloader\main.py total lines 230 
########################################################################
...
SCAN_ROOT = os.environ.get("SCAN_ROOT_PATH", "/app/data/users")
DATA_ROOT = Path(SCAN_ROOT)
...
def ensure_dependency():
    try:
        ...
        print("📦 [Neural-Sync] Injecting missing conduits: yt-dlp...", flush=True)

        raise ImportError("Dependency 'yt-dlp' missing. Core Auto-Heal should take over.")
...
def resolve_path(path_str, user_id):
    ...
    if clean_path.startswith(str(DATA_ROOT / real_id)):
