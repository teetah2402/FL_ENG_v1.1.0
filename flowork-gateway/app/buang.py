...
# File NAME : C:\flowork-nano\flowork-gateway\app\buang.py total lines 321 
########################################################################
...
class User(db.Model):
    ...
    preferences = db.Column(db.JSON, nullable=True, server_default='{}')
    engines = db.relationship('RegisteredEngine', back_populates='owner')
    ...
    episodes = db.relationship("Episode", back_populates="user", lazy="dynamic", cascade="all, delete-orphan")
    agent_sessions = db.relationship("AgentSession", back_populates="user", lazy="dynamic", cascade="all, delete-orphan")
...
class RegisteredEngine(db.Model):
    ...
    created_at = db.Column(db.DateTime, server_default=db.func.now())
    internal_url = db.Column(db.String, nullable=True) # Essential for Core Proxying
    ...
    @property
    def token(self): return self.engine_token_hash
...
class Job(db.Model):
    ...
    __table_args__ = (Index('idx_jobs_status_prio', 'status', 'priority', 'available_at', 'created_at'),)
