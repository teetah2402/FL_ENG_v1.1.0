...
# File NAME : C:\FLOWORK\flowork-gateway\app\routes\engine.py total lines 139 
########################################################################

from flask import Blueprint, request, jsonify, g, current_app
import uuid
import os
from functools import wraps # [ADDED BY FOWORK DEV] Fix missing wraps import
...
from app.globals import globals_instance # [ADDED BY FOWORK DEV] Sync with RAM
...
def register_engine():
    ...
    if existing_engine:
        if existing_engine.user_id == user.id:
            ...
            new_raw_token = f"eng_{secrets.token_hex(32)}"
            existing_engine.engine_token_hash = generate_password_hash(new_raw_token)

            db.session.commit()
            current_app.logger.info(f"Engine {engine_id} re-registered/updated (NEW TOKEN GENERATED) by owner {user.public_address}")

            return jsonify({
                "message": "Engine re-registered successfully. Token rotated.",
                "token": new_raw_token # [RETURN NEW TOKEN]
            }), 200
        else:
...
def engine_auth_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        engine_id_hint = request.headers.get('X-Engine-ID')
        ...
        if engine_id_hint:
            e = session.query(RegisteredEngine).filter_by(id=engine_id_hint).first()
            if e and check_password_hash(e.engine_token_hash, token):
                engine = e
        ...
        if getattr(engine, 'is_deleted', False): # Use getattr for safety
        ...
        if engine.id not in globals_instance.engine_manager.engine_url_map:
            globals_instance.engine_manager.engine_url_map[engine.id] = "http://flowork_core:8989"
...
def get_auth_whitelist():
    ...
    try:
        owner_address = engine.owner.public_address if hasattr(engine, 'owner') else None

        if not owner_address:
            owner = User.query.get(engine.user_id)
            owner_address = owner.public_address if owner else None
