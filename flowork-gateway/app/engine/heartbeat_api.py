...
# File NAME : C:\FLOWORK\flowork-gateway\app\engine\heartbeat_api.py total lines 72 
########################################################################
...
from app.globals import globals_instance # [ADDED BY FOWORK DEV] To sync RAM
...
def heartbeat():
    ...
    try:
        ...
        if engine:
            ...
            if engine_id not in globals_instance.engine_manager.active_engine_sessions:
                globals_instance.engine_manager.active_engine_sessions[engine_id] = "HTTP_LIVE"
                current_app.logger.info(f"[Heartbeat] Engine {engine_id} synced to RAM via HTTP Pulse.")
...
def cleanup_stale_engines():
    ...
    try:
        if db.engine.dialect.name == 'sqlite':
            sql = f"UPDATE registered_engines SET status = 'offline' WHERE last_seen < datetime('now', '-{ttl_seconds} seconds')"
            db.session.execute(db.text(sql))
            db.session.commit()
        elif db.engine.dialect.name == 'postgresql':
            sql = f"UPDATE registered_engines SET status = 'offline' WHERE last_seen < NOW() - INTERVAL '{ttl_seconds} seconds'"
            db.session.execute(db.text(sql))
            db.session.commit()
